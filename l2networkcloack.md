Below is the **L2 “Network Cloak” Protocol** 

It **sits on top of L1** (CLEAN LAPTOP + CLEAN PHONE already exist).

---

##### 0\. What You’re Building at L2
...
Everything else (DIRTY devices, public Wi-Fi without VPN) = synthetic / untrusted.
...
> `VPN PROVIDER: \_\_\_\_\_\_\_\_\_\_\_`
> `VPN ACCOUNT / NUMBER: \_\_\_\_\_\_\_\_\_\_\_`

Keep this with your L0/L1 notes (not next to the router).

---

##### 2\. Name Your Networks (Mental Map)
...
##### 3\. BASIC – Install VPN on CLEAN LAPTOP (Debian)
...
2. Go to your provider’s official site:
   ...
   * Mullvad: type `mullvad vpn` and pick the official `.net` domain.
   * IVPN: type `ivpn` and pick the official `.net` / `.com` domain.

3. Find **Linux / Debian** download:

   * Either a `.deb` package **or**
   * A repo install instruction (copy/paste command)

4. Download and install:

   * If `.deb`: double-click → Software Installer → Install
   * Or follow the simple Linux instructions on their site (usually 1–2 copy/paste commands)
...
4. It should now show **your account is active**.
...
1. Turn ON:
   ...
   * `Auto-connect` to a server (can choose “Best” or a nearby country)
...
2. Open browser.
3. Search `what is my ip` or use provider’s “Check” page (e.g. Mullvad’s check page).
4. Confirm:
...
##### 4\. BASIC – Install VPN on CLEAN PHONE

###### 4.1 For GrapheneOS / Android CLEAN PHONE

On **CLEAN PHONE**:

1. Open browser or app store you trust (GrapheneOS “Apps”, F-Droid, or provider’s site).
...
Open the phone’s system **Network / VPN** settings:
...
###### 4.2 For TEMP CLEAN (stock Android / iOS)

* Do the **same steps**: install official VPN app from store.
* For Android: set **Always-on VPN** + **Block connections without VPN**.
* For iOS: set VPN to connect automatically where possible (no perfect kill switch – note below).
...
##### 5\. BASIC – Simple Use Modes (Traffic Lights)

Make a small table on paper and tape near CLEAN LAPTOP:
...
> → Not allowed (except maintenance you \*\*explicitly\*\* decide)

Rule:

* CLEAN LAPTOP + CLEAN PHONE operate only in **PRIVATE** or **GHOST** mode.
* DIRTY devices can be used without VPN if needed (bank, KYC, streaming).
...
##### 6\. ADVANCED – Install Tor Browser on CLEAN LAPTOP

On **CLEAN LAPTOP**:

1. Open browser (with VPN ON).
2. Search `Tor Browser Linux download`.
3. Go to official Tor Project site (`torproject.org`).
4. Download Tor Browser for Linux.
5. Follow the simple install steps (usually: extract and run `start-tor-browser`).

First run:

1. Start Tor Browser.
2. Choose `Connect` (no special bridges unless you need them).
...
* Option 2: Use CLEAN PHONE as **hotspot**:
  ...
  3. VPN on laptop ON as well (double cloak but fine).
...
* Same: use the app’s DNS settings (if visible) or accept defaults
...
##### 9\. What NEVER to Do on L2 (Hard Rules)
...
1. Never open CLEAN LAPTOP or CLEAN PHONE browser **with VPN OFF** (except for rare, explicitly chosen maintenance tasks).
...
##### 10\. Weekly \& Monthly L2 Checklists
...
* \[ ] Check home router Wi-Fi password is still non-default and not shared widely.
* \[ ] Try connecting CLEAN LAPTOP to a `PUBLIC NET` (café, library) and confirm:
...
##### 11\. Edge Cases and Emergency Branches
...
* If you briefly browsed with VPN OFF:
  ...
  * Do not panic; note which sites were opened.
...
* If you logged into social media, KYC exchange, or bank:
  ...
  * That CLEAN device is now **contaminated**:
    ...
    * Option B: Fully wipe and rebuild OS, re-apply L1 + L2 protocol from scratch.
...
If all 7 are true, **L2 Network Cloak** is wrapped around your **L1 Device Shell** and **L0 Secret Box** and all traffic from CLEAN devices now passes through a predictable, controlled privacy pipeline.
